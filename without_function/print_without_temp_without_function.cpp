/*print the closest temperature to the
 upper or lower with out the number 1,4 and 7*/
#include <iostream>
#include <cmath>
#include <string>
...
int main() {
    ...
    int ans[2] = {temperature,temperature};
    int dummy_temp = temperature;
    for (int i = 2; i >= 0; i--) {
        int dig = dummy_temp / pow(10, i); 
        if (dig == 1 || dig == 4 || dig == 7) {
            ans[0] = temperature - dummy_temp +(dig + 1)*pow(10,i);
            ans[1] = temperature - dummy_temp +(dig )*pow(10,i) - 1;
            break;
        }
